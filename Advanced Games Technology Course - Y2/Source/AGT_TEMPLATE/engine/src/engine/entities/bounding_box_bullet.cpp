...
void engine::bounding_box::set_box(float width, float height, float depth, glm::vec3 position, bool Static)
{
	m_width = width;
	m_height = height;
	m_depth = depth;
	...
	if (Static)
		m_position = position - glm::vec3(m_width / 2.f, 0.0f, m_depth / 2.f);
	else
		m_position = position;
...
void engine::bounding_box::on_update(glm::vec3 position, float rotation, glm::vec3 axis, bool Static)
{
	if (Static)
		m_position = position - glm::vec3(m_width / 2.f, 0.f, m_depth / 2.f);
	else
		m_position = position;
...
void engine::bounding_box::on_render(float r, float g, float b, const engine::ref<engine::shader>& shader, bool Static)
{
