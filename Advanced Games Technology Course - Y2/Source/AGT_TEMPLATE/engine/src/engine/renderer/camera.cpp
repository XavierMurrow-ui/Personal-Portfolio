...
/*void engine::perspective_camera::on_update(const timestep& timestep)
{
    ...
    update_rail(timestep);//Forces camera to follow mouse/front vector
	update_camera_vectors();

    //Tests for mouse left and right clicks and the spacebar
    if (engine::input::mouse_button_pressed(0) && s_movement_speed <= 0.1f)
        set_movement_speed(s_movement_speed += 0.001f);
    else if (engine::input::mouse_button_pressed(1) && s_movement_speed >= 0)
        set_movement_speed(s_movement_speed -= 0.0005f);
    else if (engine::input::key_pressed(engine::key_codes::KEY_SPACE) && s_movement_speed > 0)
        set_movement_speed(0);
    else if (engine::input::key_pressed(engine::key_codes::KEY_SPACE) && s_movement_speed <= 0)
        set_movement_speed(SPEED);
    //m_position.y = y_value;

    m_position.y = glm::clamp(m_position.y, 1.0f, 11.0f);//Clamps the cam in the y axis between 1 and 11 so the player cannot clip through the terain
...
{ 
    ...
    //Created this to use space and shift to move the camera up and down respectively
    //is not used in interim build
    if (direction == up)
        m_position.y += s_movement_speed * ts;
    else if (direction == down)
        m_position.y -= s_movement_speed * ts;
...
{ 
    ...
    //m_view_mat = glm::inverse(transform); 
   m_view_mat = glm::lookAt(m_position, m_position + m_front_vector, m_up_vector);
    //m_position = glm::vec3(0.f, 20.f, 0.f);
...
void engine::perspective_camera::set_view_matrix_up(glm::vec3 position, glm::vec3 look_at, glm::vec3 upVec)
{
    m_position = position;
    m_front_vector = look_at;
    m_view_mat = glm::lookAt(m_position, look_at, upVec);
    m_view_projection_mat = m_projection_mat * m_view_mat;
}
...
void engine::perspective_camera::update_rail(const timestep& ts) {
    m_position += s_movement_speed * normalize(m_front_vector);//forces camera to follow the front vector the mouse creates
}
